...
$stmt = $mysqli->query("SELECT * FROM products ORDER BY created_at DESC LIMIT 30");
...
<?php foreach($products as $p): ?>
  <div class="bg-white p-4 rounded shadow">
    <img src="/online_store/assets/img/<?php echo htmlspecialchars($p['image'] ?: 'default.jpg'); ?>"
     alt="<?php echo htmlspecialchars($p['name']); ?>"
     class="w-full h-48 object-cover rounded block bg-gray-100">

    <h3 class="mt-2 font-semibold"><?php echo h($p['name']); ?></h3>
    <p class="text-sm text-gray-500"><?php // ใช้โค้ดนี้แทนของเดิมในบรรทัดที่ 15
echo isset($row['shop_name']) ? $row['shop_name'] : ''; ?></p>
